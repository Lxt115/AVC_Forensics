...
def get_files_from_split(split):
    ...
    # path_real = "data/datasets/Forensics/RealFF/c23/cropped_mouths"
    # path_fake = "data/datasets/Forensics/Deepfakes/c23/cropped_mouths"
    # files_real = []
    # files_fake = []
    # for files in os.listdir(path_real):
    #     files_real.append(files)
    # for files in os.listdir(path_fake):
    #     files_fake.append(files)

    return files_real, files_fake


def dfdc_get_files(path):
    data_name = path.columns.values
    value = path.values[0]
    files_real = []
    files_fake = []
    for i in range(len(data_name)):
        if value[i] == 0:
            files_real.append(data_name[i])
        else:
            files_fake.append((data_name[i]))
...
def load_model(load_path, model, optimizer=None, allow_size_mismatch=False):
    ...
    model.load_state_dict(loaded_state_dict, strict=allow_size_mismatch)
